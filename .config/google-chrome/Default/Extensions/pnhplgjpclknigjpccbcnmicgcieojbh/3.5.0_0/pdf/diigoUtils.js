...
var diigoUtils = {
  ...
  getUsername: async function () {
    const globalData = await getStorage("globalData");
  ...
  getRequestURL: function () {
    ...
    return (
      server +
      "/chappai/pv=#{pv}/ct=pj/cv=#{cv}/user=#{user}/cmd=#{cmd}/from=#{from}/"
    );
  },
  ...
  evalTpl: function (tpl, object) {
    ...
    return gsub(tpl, tplPattern, function (match) {
      ...
      while (match != null) {
        var comp =
          match[1].indexOf("[") === 0 ? gsub(match[2], "\\\\]", "]") : match[1];
        ctx = ctx[comp];
  ...
  showTpl: function (s, data) {
    ...
    return s.replace(/(^|.|\r|\n)(#\{(.*?)\})/g, function (m, m1, m2, m3) {
      ...
      while (match != null) {
        var comp =
          match[1][0] == "[" ? match[2].replace(/\\\\]/g, "]") : match[1];
        ctx = ctx[comp];
        if (null == ctx || "" == match[3]) break;
        expr = expr.substring(
          "[" == match[3] ? match[1].length : match[0].length
    ...
    function ConvertToWordArray(sMessage) {
      ...
      while (lByteCount < lMessageLength) {
        ...
        lWordArray[lWordCount] =
          lWordArray[lWordCount] |
          ((isStr ? sMessage.charCodeAt(lByteCount) : sMessage[lByteCount]) <<
            lBytePosition);
        lByteCount++;
      }
      lWordCount = (lByteCount - (lByteCount % 4)) / 4;
