// var GlobalData = await getStorage("globalData");
...
function diigoData(url, title, file_md5, data) {
  ...
  diigoUtils.getUsername().then((user) => {
    that.user = user || null;
    ...
    diigoNetwork.bm_loadBookmark(
      this,
      "annotations bookmarkInfo",
      function (res) {
    ...
    diigoNetwork.loadMyStuff(
      "myTags  myBookmarkLists myGroups",
      this,
      function (data) {
        // update lists
        console.log(data);
...
var diigoAnnotationHelper = (function diigoAnnotationHelper() {
  ...
  diigoAnnotationHelper.parseRange = function parseRange(pageIndex, rg) {
    ...
    var offsetX = parentRc.left,
      offsetY = parentRc.top;
    ...
    if (d_list.length == 0) {
      return {
        rects: [
          [
            rects[0].left - offsetX,
            rects[0].top - offsetY,
            rects[0].left - offsetX + rects[0].width,
            rects[0].top - offsetY + rects[0].height
          ]
        ],
        text: p.innerText
      };
    ...
    for (var i = 0; i < d_list.length; i++) {
      ...
      var width =
        parseFloat(div.getAttribute("data-width")) *
        parseFloat(div.getAttribute("data-transform-scale-x"));
  ...
  diigoAnnotationHelper.removeHighlight = function removeHighlight(h_id) {
    ...
    diigoNetwork.deleteHighlightAndStickyNote(
      h_id,
      DiigoData,
      function (resp, ct, code) {
  ...
  diigoAnnotationHelper.updateFloatnoteContent =
    function updateFloatnoteContent(f_id, content) {
      var pageIndex, ic_id;
      ...
      diigoNetwork.updateComment(
        ic_id,
        content,
        DiigoData,
        function (resp, ct, code) {
          console.log(
            "Update floatnote comment successfully. Server code: " + code
          );
        }
      );
  ...
  diigoAnnotationHelper.updateFloatnotePos = function updateFloatnotePos(
    f_id,
    pos
  ) {
    var ann, page;
    ...
    diigoNetwork.updateHighlightAndStickyNote(
      ann,
      DiigoData,
      function (resp, ct, code) {
        console.log(
          "Update floatnote position successfully. Server code: " + code
        );
      }
    );
  ...
  diigoAnnotationHelper.updateFloatnoteColor = function updateFloatnoteColor(
    f_id,
    color
  ) {
    var pageIndex;
    ...
    diigoNetwork.updateHighlightAndStickyNote(
      ...
      function (resp, ct, code) {
        console.log(
          "Update floatnote color successfully. Server code: " + code
        );
      }
    );
    page.drawDiigoFloatnotes();
  ...
  diigoAnnotationHelper.appendHighlight = function appendHighlight(
    pageIndex,
    range,
    callback
  ) {
    ...
    var diigoHighlight = {};
    diigoHighlight.id = diigoUtils.md5(
      "" +
        DiigoData.bookmark.server_url +
        pageIndex +
        text +
        quadRects +
        Date.now()
    ...
    var rect_left = quadRects[0][0],
      rect_top = quadRects[0][1],
      rect_right = quadRects[0][2],
      rect_bottom = quadRects[0][3];

    var page = PDFViewerApplication.pdfViewer._pages[pageIndex];
    for (var i = 0; i < quadRects.length; i++) {
      var rc = quadRects[i];
      var pdf_rect = page
        .getPagePoint(rc[0], rc[1])
        .concat(page.getPagePoint(rc[2], rc[3]));
    ...
    diigoHighlight.extra.rect = page
      .getPagePoint(rect_left, rect_top)
      .concat(page.getPagePoint(rect_right, rect_bottom));

    diigoHighlight.groups = [];
    diigoHighlight.comments = [];
    DiigoData.highlights.push(diigoHighlight);
    diigoNetwork.saveHighlightAndStickyNote(
      diigoHighlight,
      DiigoData,
      function (resp, ct, code) {
  ...
  diigoAnnotationHelper.updateHighlightColor = function updateHighlightColor(
    h_id,
    color
  ) {
    var pageIndex;
    ...
    diigoNetwork.updateHighlightAndStickyNote(
      ...
      function (resp, ct, code) {
        console.log(
          "Update diigo highlight color successfully. Server code: " + code
        );
      }
    );
    page.drawDiigoHighlights();
  ...
  diigoAnnotationHelper.addInlineComment = function addInlineComment(
    h_id,
    content
  ) {
    var pageIndex;
    ...
    if (update) {
      diigoNetwork.updateComment(
        ...
        function (resp, ct, code) {
          console.log(
            "Update inline comment successfully. Server code: " + code
          );
        }
      );
    } else {
      diigoNetwork.addComment(
        h_id,
        comment,
        DiigoData,
        function (resp, ct, code) {
  ...
  diigoAnnotationHelper.deleteInlineComment = function deleteInlineComment(
    h_id
  ) {
    var pageIndex;
    var ic_id;
  ...
  diigoAnnotationHelper.addFloatnote = function addFloatnote(
    pageIndex,
    pos,
    content
  ) {
    ...
    var page = PDFViewerApplication.pdfViewer._pages[pageIndex];
    var pdf_pos = page.getPagePoint(pos.left, pos.top);
    var id = diigoUtils.md5(
      "" + DiigoData.bookmark.server_url + pageIndex + content + Date.now()
    );
    var floatnote = {};
    floatnote.id = id;
    ...
    if (ann.type == 9) {
      ...
      if (pageContainer.children("a").length != 0) {
        pageContainer.children("a").each(function (i, v) {
          if (
            parseFloat($(v).data("dest").split("#")[2]) <
            parseFloat(ann.extra.rect[1])
          ) {
            elem = v;
            return false;
          }
      ...
      dest =
        ann.extra.pageIndex + 1 + "#" + ann.extra.left + "#" + ann.extra.top;
      if (elem) {
        $(
          '<a href="#" data-dest="' +
            dest +
  ...
  diigoAnnotationHelper.setupLinks = function setupLinks() {
    var that = this;
    $.each(
      DiigoData.highlights.concat(DiigoData.floatnotes),
      function (index, ele) {
        that.addLink(ele);
      }
    );
...
async function initAnnotation() {
  ...
  $("#diigolet-annMenu-colorPicker-pdf b.ann-colorItem").bind(
    "click",
    function () {
  ...
  $("#diigolet-annMenu-add").bind("click", function (e) {
    ...
    var note;
    note = $("#pageContainer" + pageIndex)
      .children("div.diigoAnnotationLayer")
      .find('div.diigo-note[data-diigo-id="' + id + '"]');
  ...
  $("#add_highlight").bind("click", function () {
    ...
    $("body")
      .removeClass("add_note")
      .toggleClass("add_highlight")
      .removeClass("yellow blue green pink")
      .addClass(DiigoData.currentColor);
  });
  ...
  $("#add_note").bind("click", function () {
    ...
    $(this).toggleClass("toggled");
    $("body")
      .removeClass("add_highlight")
      .toggleClass("add_note")
      .removeClass("yellow blue green pink")
      .addClass(DiigoData.currentColor);
  });
  ...
  $("#viewer").on("mouseup", "div.textLayer", function (e) {
    ...
    var self = this;
    var pageIndex =
      parseInt(page.attr("id").match(/pageContainer(\d+)/)[1]) - 1;
    if ($("body").hasClass("add_note")) {
      var note;
      note = page
        .children("div.diigoAnnotationLayer")
        .children("div.diigo-note");
      if (note && note.length != 0) {
        //do nothing
      } else {
        note = $(".pdf-area>div.diigo-note").clone(true);
  ...
  $(".diigolet-csm .diigolet-csm-color").hover(
    function () {
      $(this).addClass("hovered");
    },
    function () {
  ...
  $("#viewer").on(
    "mousedown",
    ".diigolet-csm-highlight,.diigolet-csm-coloritem",
    async function () {
      var selection = window.getSelection();
      if (!selection || selection.toString() == "") {
        return false;
      ...
      const { permissions } = await getStorage("globalData");
  ...
  $("#viewer").on("click", ".diigolet-csm-search", function () {
    ...
    var text = res.text;
    window.open(
      "https://www.diigo.com/search/g?q=" + encodeURIComponent(text),
      "_blank"
    );
  });
  ...
  $("#viewer").on(
    "mousedown",
    ".diigolet-csm-highlightAndComment",
    async function () {
      var selection = window.getSelection();
      if (!selection || selection.toString() == "") {
        return false;
      ...
      diigoAnnotationHelper.appendHighlight(pageIndex, rg, function (id) {
        ...
        note = page
          .children("div.diigoAnnotationLayer")
          .children('div.diigo-note[data-diigo-id="' + id + '"]');
        if (note && note.length != 0) {
          //do nothing
        } else {
          note = $(".pdf-area>div.diigo-note").clone(true);
  ...
  $(".diigo-note").hover(
    ...
    function (e) {
      ...
      if (
        $(this).attr("data-origin") &&
        $(this).attr("data-origin") != $(this).find(".diigo-note-input").val()
      ) {
        //do nothing
      } else {
        var self = this;
  ...
  $(".diigo-note-save").bind("click", function (e) {
    ...
    if (type == "stickynote") {
      ...
      if (id) {
        ...
        diigoAnnotationHelper.addFloatnote(
          pageIndex,
  ...
  $("#save-pdf").on("click", async function () {
    const { permissions } = await getStorage("globalData");
    if (!(permissions && permissions.pdfPermission.pdfPermission)) {
      diigoModal.hide();
      diigoModal.show("upgrade");
  ...
  var { myTags } = await getStorage("globalData");
  var OriginTags = myTags;
  var tags = [];
  ...
  $("#screenshot").bind("click", function (e) {
    ...
    if ($("div#diigo-chrome-installed").length > 0) {
      ...
      if (window.navigator.userAgent.match(/firefox/i)) {
        $("#popup-message")
          .html(
            'Get <a href="https://www.diigo.com/tools/toolbar?client=ff">diigo firefox toolbar</a> to take capture.'
          )
          .removeClass("hidden");
      } else {
        $("#popup-message")
          .html(
            'Get <a onclick="installExt(event);" href="https://chrome.google.com/webstore/detail/diigo-web-collector-captu/oojbgadfejifecebmdnhhkbhdjaphole">diigo chrome extension</a> to take capture.'
          )
          .removeClass("hidden");
      }
  ...
  $("#diigobm-list-addBtn").on("click", async function () {
    var that = this;
    ...
    var { outliners } = await getStorage("globalData");
    var length = outliners.length;

    for (i = 0; i < length; i++) {
      lists.push(outliners[i].title);
    }
    ...
    Web.createList(listName).then(function (data) {
      ...
      $listSelect
        .find(".content")
        .text(data.newOutliner.title)
        .attr("data-id", data.newOutliner.id);
  ...
  $("#download_ann").click(function () {
    ...
    $.ajax({
      ...
      success: function (data, res) {
        ...
        if (data.code == 1) {
          message =
            "Submitted! You can go to <a target='_blank' href='/tools/pdf_export?t=" +
            data.time +
            "'><strong>tools->PDF Export</strong></a> to download your file when the file is processed.";
          ...
          message =
            "You have just submitted a downloading request. \nCheck your downloads <a target='_blank' href='/tools/pdf_export?t=" +
            data.time +
            "'><strong>tools->here</strong></a>.";
          $("#popup-message").html(message).removeClass("hidden");
          tt = window.setTimeout(function () {
            $("#popup-message").addClass("hidden");
          }, 5000);
          ...
          message =
            "You have no annotations. Just click the dowload button on the right.";
          $("#popup-message")
            .addClass("failed")
            .html(message)
            .removeClass("hidden");
...
function share(share_way) {
  ...
  if (share_way == "twitter") {
    ...
    to_url =
      "mailto:?subject=Check this pdf&body=" +
      encodeURIComponent(share_text) +
      "%0A" +
      encodeURIComponent(share_link);
  }
  ...
  window.open(
    to_url,
    "",
    "left=300,top=100,width=500,height=500,toolbar=0,resizable=0,menubar=0,location=0,status=0,scrollbars=yes"
  );
}
function inList(id) {
...
function updateGroups(groups) {
  ...
  if (_teams.length) {
    ...
    diigoUtils.forEach(_teams, function (index, o) {
      var displayName = inGroup(o.name)
        ? o.displayName + " (shared)"
        : o.displayName;
      $(
  ...
  if (_groups.length) {
    ...
    diigoUtils.forEach(_groups, function (index, l) {
      var displayName = inGroup(l.name)
        ? l.displayName + " (shared)"
        : l.displayName;
      $(
        '<div class="menu-item" data-id="' +
          l.name +
...
function updateLists(data) {
  ...
  if (outliners.length) {
    ...
    diigoUtils.forEach(outliners, function (index, o) {
      ...
      $(
        '<div class="menu-item" data-id="' + o.id + '">' + title + "</div>"
      ).appendTo(container);
    });
  }
  ...
  if (lists.length) {
    ...
    diigoUtils.forEach(lists, function (index, l) {
      ...
      $(
        '<div class="menu-item" data-id="' + l.id + '">' + title + "</div>"
      ).appendTo(container);
    });
  }
}
