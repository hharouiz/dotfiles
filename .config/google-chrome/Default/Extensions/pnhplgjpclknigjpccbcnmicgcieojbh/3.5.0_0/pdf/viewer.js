...
var FontMetrics = (function FontMetricsClosure() {
  ...
  FontMetrics.prototype = {
    ...
    getFontAscent: function FontMetricsDetFontAscent(
      fontHeight,
      fontStyle,
      str
    ) {
...
var PDFPageView = (function PDFPageViewClosure() {
  ...
  PDFPageView.prototype = {
      ...
      if (this.annotationsLayerFactory) {
        if (!this.annotationLayer) {
          this.annotationLayer =
            this.annotationsLayerFactory.createAnnotationsLayerBuilder(
              div,
              this.pdfPage
            );
        }
    ...
    drawDiigoHighlights: function drawDiigoHighlights() {
      ...
      $("#annotationView #pageLinkContainer" + self.id)
        .children(".highlight-link,.stickynote-link")
        .remove();
      ...
      if (!self.diigoAnnotationLayer) {
        ...
        $(self.diigoAnnotationLayer)
          .children(".div_highlight,.stickynote")
          .remove();
      }
      ...
      for (i = 0, ii = highlights.length; i < ii; i++) {
        ...
        $.each(quadRects, function (index, rect) {
          ...
          if (index == quadRects.length - 1 && data.comments.length) {
            ...
            var note = $(".pdf-area>div.diigo-note").clone(true);
            note.addClass("stickynote");
            note
              .find(".diigo-note-header span.diigo-note-title")
              .html("Stickynote");
            note.find(".diigo-note-input").val(data.comments[0].content);
            note
              .attr("diigo-note-type", "stickynote")
              .attr("data-diigo-id", data.id)
              .attr("data-origin", data.comments[0].content)
              .removeClass("yellow blue green pink")
              .addClass(data.extra.color);
            $(element).append(note);
            $(menu).hover(
              function (e) {
                ...
                note
                  .find(".diigo-note-input")
                  .val(note.attr("data-origin").replace(/<br \/>/g, "\n"));
                ...
                note
                  ...
                  .draggable({ handle: ".diigo-note-header", opacity: 0.8 })
                  .fadeIn("fast");
              },
              function (e) {
                var self = this;
                ...
                if (e.relatedTarget.id != "diigolet-annMenu-arrow-pdf") {
                  window.setTimeout(function () {
                    if (
                      !$(self).hasClass("hovered") &&
                      !$("#diigolet-annMenu").hasClass("hovered")
                    ) {
                      $("#diigolet-annMenu").hide();
                    }
                  }, 300);
          ...
          $(rect_div).hover(
            function (e) {
              ...
              var pageIndex = self.id;
              if (
                !$("#diigolet-annMenu").hasClass("show") ||
                $("#diigolet-annMenu").attr("data-diigo-id") != ann_id
              ) {
                if (
                  $(this).siblings(".diigo-note").length &&
                  $(this).siblings(".diigo-note").is(":visible")
                ) {
                  return false;
                }
                $("#diigolet-annMenu-currentColor")
                  .removeClass("yellow blue green pink")
                  .addClass(color);
                $(
                  "#diigolet-annMenu-colorPicker-pdf #diigolet-context-" + color
                )
                  .addClass("colorchecked")
                  .siblings()
                  .removeClass("colorchecked");
                $("#diigolet-annMenu-colorPicker-pdf").addClass("hidden");
                $("#diigolet-annMenu-copy").attr(
                  "data-clipboard-text",
                  highlightDiv.attr("data-clipboard-text")
                );
                ...
                window.setTimeout(function () {
                  ...
                  $("#diigolet-annMenu")
                    ...
                    .attr("data-diigo-id", ann_id)
                    .attr("data-diigo-color", color)
                    .css({
                      left: left + "px",
                      top: e.pageY - 55 + "px"
                    })
      ...
      if (
        $("#annotationView #pageLinkContainer" + self.id).children("a")
          .length == 0
      ) {
    ...
    drawDiigoFloatnotes: function drawDiigoFloatnotes() {
      ...
      if (!self.diigoAnnotationLayer) {
        ...
        $(self.diigoAnnotationLayer)
          .children(".div_floatnote,.floatnote")
          .remove();
      }
      var i, ii;
      for (i = 0, ii = floatnotes.length; i < ii; i++) {
        ...
        element.setAttribute(
          "data-origin",
          data.comments[0].content.replace(/<br \/>/g, "\n")
        );
        var pos = self.viewport.convertToViewportPoint(
          data.extra.left,
          data.extra.top
        );
        ...
        $(element).draggable({
          ...
          stop: function (e, ui) {
            ...
            diigoAnnotationHelper.updateFloatnotePos(id, {
              left: left,
              top: top
            });
          }
        ...
        $(element).hover(
          function () {
            ...
            note
              .attr("data-diigo-id", $(this).attr("data-diigo-id"))
              .attr("data-origin", $(this).attr("data-origin"))
              .attr("diigo-note-type", "floatnote")
              .removeClass("yellow blue green pink")
              .addClass($(this).attr("data-diigo-color"))
              .css({
                left: parseInt($(this).css("left")) + 15 + "px",
                top: parseInt($(this).css("top")) + 10 + "px"
              });
            note
              .find("textarea.diigo-note-input")
              .val($(this).attr("data-origin"));
            note.removeClass("new");
            $(this).parent().append(note);
            note.fadeIn("fast", function () {
              $(this).draggable({
                handle: ".diigo-note-header",
                containment: "parent",
                opacity: 0.8
              });
            });
...
var TextLayerBuilder = (function TextLayerBuilderClosure() {
  ...
  TextLayerBuilder.prototype = {
      ...
      for (var i = 0; i < this.textDivs.length; i++) {
        if ($(this.textDivs[i]).find("span").length == 0) {
          ...
          $(this.textDivs[i])
            ...
            .each(function () {
              var left = Math.abs(
                $(_textLayerDiv).offset().left - $(this).offset().left
              );
    ...
    buildSentenceDOM: function TextLayerBuilder_buildSentenceDOM(
      textDiv,
      geom,
      style,
      ctx
      ...
      if (
        geom.structure &&
        geom.structure.length > 0 &&
        geom.coalesceType === "sentence"
      ) {
        for (var j = 0, jj = geom.structure.length; j < jj; j++) {
          ...
          span.style.letterSpacing =
            (styleWidth - word.canvasWidth) / word.str.length + "px";
    ...
    appendText: function TextLayerBuilder_appendText(geom, styles, ctx) {
      ...
      var fontAscent = this.fontMetrics.getFontAscent(
        fontHeight.toFixed(3),
        style.fontFamily
      );
      ...
      geom.textScale =
        (geom.vertical ? geom.height : geom.width) /
      ...
      textDiv.classList.add(
        isAllWhitespace(geom.str) ? "parent-whitespace" : "parent-word"
      );
      ...
      if (angle) {
        transform = "rotate(" + angle * (180 / Math.PI) + "deg) " + transform;
      }
      CustomStyle.setProp("transform", textDiv, transform);
    ...
    compileTextContent: function TextLayerBuilder_compileTextContent() {
      ...
      var rotate =
        (360 +
          (Math.atan2(this.viewport.transform[1], this.viewport.transform[0]) /
            Math.PI) *
            180) %
        360;
      // Lock it into an integer so we can compare it.
      rotate =
        Math.abs(rotate) < EPSILON
          ? 0
          : Math.abs(rotate - 90) < EPSILON
          ? 90
          : Math.abs(rotate - 180) < EPSILON
          ? 180
          : Math.abs(rotate - 270) < EPSILON
          ? 270
          : rotate;
      ...
      for (i = dontDoPadding ? len : 0; i < len; i++) {
        ...
        var farRight =
          geom.right !== null
            ? getDivPosition(textItems[geom.right]).left
            : bounds.right;
        var farBottom =
          geom.bottom !== null
            ? getDivPosition(textItems[geom.bottom]).top
            : bounds.bottom;

        // Update Padding. Apply textScale as appropriate (horizontal only).
        divi.style.paddingRight =
          Math.abs(farRight - right) / geom.textScale + "px";
        ...
        if (geom.left === null) {
          ...
          if (
            ...
            (leftItem.bottom === null ||
              diffY(getDivPosition(textItems[leftItem.bottom]).top, bottom) >=
                0)
          ) {
...
var AnnotationsLayerBuilder = (function AnnotationsLayerBuilderClosure() {
  ...
  AnnotationsLayerBuilder.prototype =
    /** @lends AnnotationsLayerBuilder.prototype */ {
       ...
       * @param {PageViewport} viewport
       */
      setupAnnotations: function AnnotationsLayerBuilder_setupAnnotations(
        viewport
      ) {
        ...
        pdfPage.getAnnotations().then(function (annotationsData) {
          ...
          if (self.div) {
            ...
            for (i = 0, ii = annotationsData.length; i < ii; i++) {
              ...
              element = PDFJS.AnnotationUtils.getHtmlElement(
                data,
                pdfPage.commonObjs
              );
...
var PDFViewerApplication = {
  ...
  setTitle: function pdfViewSetTitle(title) {
    ...
    $("#toolbarViewer").find(".title").text(title);
...
function webViewerInitialized() {
  ...
  document
    .getElementById("viewDiigoAnnotation")
    .addEventListener("click", function () {
...
/* luo added*/
var isGoogleDrive = 0; //1 googledrive, 2, Box;3:droBox
var Gtoken = null;
var GOOGLE_DRIVE_FILE_URL = "https://content.googleapis.com/drive/v2/files",
...
var Box_Acesstoken = null;

function _requestWithOauth(url, data) {
...
function _request(url, method, data, headers) {
  return new Promise(function (resolve, reject) {
    var jqxhr = $.ajax({
      type: method,
      data: data,
...
function closeUploadView() {
  $("#upload-view").removeClass("active");
  $("#external-url-input").val("");
  $("#pdf-file").val("");
}

function openFilePicker() {
  openView("file-picker");
  $("#upload-view-close").hide();
  $("#file-picker-loading").show();
  buildFilePicker();
}

function openBoxFilePicker() {
  openView("file-picker");
  $("#upload-view-close").hide();
  $("#file-picker-loading").show();
  BoxOAuth.init(currentChannel);
  BoxOAuth.loadTokenInfo().then((data) => {
    if (data) {
      buildBoxItem(data);
    } else {
      BoxOAuth.tryGetAuthCode();
    }
  });
}
...
function _getFileList() {
  return _requestWithOauth(GOOGLE_DRIVE_FILE_URL, {
    q: 'mimeType="application/pdf"'
  });
}

function _getGoogleUserInfo() {
  return _requestWithOauth(GOOGLE_PROFILE_URL, {}).then(function (info) {
    console.log(info);
    return _getUserPhoto(info.picture, info);
  });
}

function _getUserPhoto(url, info) {
  return new Promise(function (resolve, reject) {
    var xhr = new XMLHttpRequest();
    xhr.responseType = "blob";
    xhr.onload = function () {
      console.log(window.URL.createObjectURL(xhr.response));
      info.userPhoto = window.URL.createObjectURL(xhr.response);
      resolve(info);
    };
...
function initFilePicker() {
  return getGoogleToken().then(function () {
    return Promise.all([_getGoogleUserInfo(), _getFileList()]);
  });
}

// open: function pdfViewOpen(file, scale, password,
//                            pdfDataRangeTransport, args)
function openBoxPdf(file_id, access_token, title) {
  Box_Acesstoken = access_token;
  var url;
  var args;
  if (currentChannel == Box) {
  ...
  } else if (currentChannel == Dropbox) {
    var data = {
      path: file_id
    };
  ...
  PDFView.open(url, 0, null, null, args);
  if (title) {
    PDFViewerApplication.setTitleUsingUrl(title);
  }
}

function buildBoxItem(token) {
  Box_Acesstoken = token;
  var url;
  var method;
  var headers;
  var parameters;
  if (currentChannel == Box) {
    url = "https://api.box.com/2.0/search?query=pdf&file_extensions=pdf";
    method = "GET";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var container = $("#file-picker-container").empty();
      ...
      if (items.length) {
        items.forEach(function (item, index, arr) {
          ...
          if (currentChannel == Box) {
            title = item.name;
            itemid = item.id;
            showtime = item.modified_at.substring(0, 10);
          } else if (currentChannel == Dropbox) {
            title = item.metadata.name;
            itemid = item.metadata.id;
            showtime = item.metadata.server_modified.substring(0, 10);
          }
          var item = diigoUtils.evalTpl(GOOGLE_ITEM_TEMPLATE, {
            NAME: title,
            // URL: item.downloadUrl.replace('&e=download&gd=true', '&e=view'),
            URL: itemid,
            TIME: showtime
          });
          ...
          $item
            ...
            .on("dblclick", function () {
              openBoxPdf($(this).data("url"), token, $(this).data("title"));
              closeFilePicker();
              closeUploadView();
            });
...
function buildFilePicker() {
  initFilePicker().then(function (info) {
    console.log(info);
    var email = info[0].email,
      displayName = info[0].name,
      items = info[1].items,
      userPhoto = info[0].userPhoto;

    console.log(items);

    $("#user-photo").attr("src", userPhoto);
    $("#file-picker-username").text(displayName);
    $("#file-picker-email").text(email);
    ...
    if (items.length) {
      items.forEach(function (item, index, arr) {
        var item = diigoUtils.evalTpl(GOOGLE_ITEM_TEMPLATE, {
          ...
          TIME: item.modifiedDate.substring(0, 10)
        });
...
function _failture(e) {
  console.log(e);
}

function openView(panel) {
  $("#" + panel + "-view").addClass("active");
}

function closeFilePicker() {
  $("#file-picker-view").removeClass("active");
  $("#upload-view-close").show();
  $("#no-pdf-gdrive").hide();
}
...
$(document).ready(function () {
  window.GOOGLE_ITEM_TEMPLATE = $("#google-item-template").html();
  $("#open-file-picker").on("click", function () {
    currentChannel = GoogleDrive;
    openFilePicker();
  });

  $("#open-box_file-picker").on("click", function () {
    currentChannel = Box;
    openBoxFilePicker();
  });
  $("#open-dropbox_file-picker").on("click", function () {
    currentChannel = Dropbox;
    openBoxFilePicker();
  });
  ...
  function downloadWithAnnotations2() {
    // $.ajax({
    //     type: 'GET',
    //     url: 'https://www.diigo.com/share_annotation/test',
    //     dataType: 'binary',
    //     success: function (data) {
    ...
    //     }
    // });
    PDFView.pdfDocument.getData().then(function (data) {
  ...
  $("#upload-view-close").on("click", function () {
    closeUploadView();
  });
  $("#open-external-url").on("click", function () {
    var url = $("#external-url-input").val();
    if (url.trim() !== "") {
      finishUpload(url);
    }
  });

  $("#external-url-input").on("keydown", function (e) {
    if (e.which === 13) {
      var url = $("#external-url-input").val();
      if (url.trim() !== "") {
        finishUpload(url);
  ...
  function finishUpload(url, filename) {
    DEFAULT_URL = url;
    openPdf(url);
  ...
  $("#file-picker-close").on("click", function () {
    closeFilePicker();
    $("#upload-view-close").hide();
  });
  $("#google-cancel").on("click", function () {
    closeFilePicker();
    $("#upload-view-close").hide();
  });

  $("#google-select").on("click", function (e) {
    e.stopPropagation();
    if (currentChannel == GoogleDrive) {
      openPdf($(this).data("url"));
    } else {
      openBoxPdf($(this).data("url"), Box_Acesstoken, $(this).data("title"));
    }
    closeFilePicker();
    closeUploadView();
  });
