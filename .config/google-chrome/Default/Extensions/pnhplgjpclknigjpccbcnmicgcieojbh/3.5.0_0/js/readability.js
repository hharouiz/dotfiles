...
Readability.prototype = {
  ...
  REGEXPS: {
    unlikelyCandidates:
      /banner|combx|comment|community|disqus|extra|foot|header|menu|modal|related|remark|rss|share|shoutbox|sidebar|skyscraper|sponsor|ad-break|agegate|pagination|pager|popup/i,
    okMaybeItsACandidate: /and|article|body|column|main|shadow/i,
    positive:
      /article|body|content|entry|hentry|main|page|pagination|post|text|blog|story/i,
    negative:
      /hidden|^hid$| hid$| hid |^hid |banner|combx|comment|com-|contact|foot|footer|footnote|masthead|media|meta|modal|outbrain|promo|related|scroll|share|shoutbox|sidebar|skyscraper|sponsor|shopping|tags|tool|widget/i,
    extraneous:
    ...
    normalize: /\s{2,}/g,
    videos:
      /\/\/(www\.)?(dailymotion|youtube|youtube-nocookie|player\.vimeo)\.com/i,
  ...
  _prepArticle: function (articleContent) {
    ...
    this._removeNodes(
      articleContent.getElementsByTagName("p"),
      function (paragraph) {
  ...
  _checkByline: function (node, matchString) {
    ...
    if (
      (rel === "author" || this.REGEXPS.byline.test(matchString)) &&
  ...
  _findBaseUrl: function () {
    ...
    for (var i = 0, slashLen = urlSlashes.length; i < slashLen; i += 1) {
      ...
      if (
        segment.match(/((_|-)?p[a-z]*|(_|-))[0-9]{1,2}$/i) &&
        (i === 1 || i === 0)
      )
  ...
  _findNextPageLink: function (elem) {
    ...
    for (var i = 0, il = allLinks.length; i < il; i += 1) {
      ...
      if (
        linkData.match(this.REGEXPS.negative) ||
        linkData.match(this.REGEXPS.extraneous)
      )
        linkObj.score -= 50;
      ...
      while (parentNode) {
        ...
        if (
          !negativeNodeMatch &&
          parentNodeClassAndId &&
          parentNodeClassAndId.match(this.REGEXPS.negative)
        ) {
      ...
      if (
        linkHref.match(/p(a|g|ag)?(e|ing|ination)?(=|\/)[0-9]{1,2}/i) ||
  ...
  _appendNextPage: function (nextPageLink) {
    ...
    articlePage.innerHTML =
      '<p class="page-separator" title="Page ' +
      this._curPageNum +
      '">&sect;</p>';
      ...
      this._ajax(pageUrl, {
        success: function (r) {
          ...
          var firstP = content.getElementsByTagName("P").length
            ? content.getElementsByTagName("P")[0]
            : null;
  ...
  _getClassWeight: function (e) {
    ...
    if (typeof e.className === "string" && e.className !== "") {
      ...
      // edit by luo
      // Edit for medium.com images
      if (
        this.REGEXPS.positive.test(e.className) ||
        (this._hasAncestorTag(e, "figure") &&
          /is-imageLoaded/.test(e.className))
      )
        weight += 25;
    }
  ...
  _clean: function (e, tag) {
    ...
    this._removeNodes(e.getElementsByTagName(tag), function (element) {
      ...
      if (isEmbed) {
        ...
        if (tag == "img") {
          if (
            !/progressiveMedia-thumbnail js-progressiveMedia-thumbnail/.test(
              element.className
            ) &&
            !/progressiveMedia-zoom js-progressiveMedia-inner/.test(
              element.className
            )
          ) {
            return false;
          ...
          if (
            !/progressiveMedia-canvas js-progressiveMedia-canvas/.test(
              element.className
            )
          ) {
            return false;
  ...
  _cleanConditionally: function (e, tag) {
    ...
    this._removeNodes(e.getElementsByTagName(tag), function (node) {
      ...
      // console.log(weight + contentScore);
      // console.log(node);
      if (weight + contentScore < 0) {
        return true;
      }
      // console.log(this._getCharCount(node, ','))
      if (this._getCharCount(node, ",") < 10) {
        ...
        if (/section-inner/.test(node.className)) {
          // console.log("IMG", img, p, this._hasAncestorTag(node, "figure"), node);
          // console.log(isList, li, p);
        }

        // edit by luo
        // for medium images
        if (
          node.getElementsByTagName("figure").length > 0 &&
          /section-inner/.test(node.className)
        ) {
          return false;
        }
        var haveToRemove =
