...
chrome.extension.onRequest.addListener(function (
  ...
  if (
    !sender.tab ||
    sender.tab.id == -1 ||
    request.action == "visible" ||
    request.action == "selected" ||
    request.action == "entire"
  ) {
  ...
  switch (request.action) {
    ...
    case "update_shortcuts":
      chrome.tabs.getAllInWindow(null, function (tabs) {
        for (var i = 0, len = tabs.length; i < len; i++) {
          ...
          if (
            !u.match(/https?:\/\/*\/*/gi) ||
            u.match(/https:\/\/chrome.google.com\/extensions/i)
          )
            continue;
    ...
    case "getAD":
      $.get("https://api.hostip.info/get_json.php", function (response) {
        ...
        var adurl =
          "https://65975.xml.premiumxml.com/xml/?fid=65975&keywords=" +
          query +
          "&user_ip=" +
          ip +
          "&ua=" +
...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  console.log(
    "test:",
    changeInfo.url,
    changeInfo.url == "https://www.awesomescreenshot.com/redirect.html#"
  );
  if (
    changeInfo.url == "chrome-extension://alelhddbbhepgpmgidjdcjakblofbmce/#"
  ) {
...
function sendRequest(where, to, req) {
  //to is a int (id)
  switch (where) {
    case "tab":
