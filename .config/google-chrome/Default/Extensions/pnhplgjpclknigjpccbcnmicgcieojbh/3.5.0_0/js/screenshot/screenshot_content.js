...
chrome.extension.onRequest.addListener(function (
  ...
  switch (request.action) {
    ...
    case "delay-capture":
      ...
      var $delayDiv = $(
        '<div id="awe_delay_div"><span></span><div id="awe_delay_cancel">Cancel</div></div>'
      )
...
function wrapperMouseDown(e) {
  if (e.button == 0) {
    ...
    function wrapperMouseMove(e) {
      ...
      (centerW = e.pageX - initX), (centerH = e.pageY - initY);
      asSize.children[0].innerHTML =
        Math.abs(centerW) + " X " + Math.abs(centerH);
    ...
    function wrapperMouseUp(e) {
      if (
        (e.pageX - initX == 0 || e.pageY - initY == 0) &&
        $("#awesome_screenshot_center").width() == 0
      ) {
        setStyle(wrapper, "background-color", "rgba(0,0,0,0)");
...
function bindCenter() {
  ...
  //bind action button
  document
    .getElementById("awesome_screenshot_action")
    .addEventListener("click", actionHandler, false);
...
function updateCorners(x, y, w, h) {
  //x:initX, w:centerW
  var topW = w >= 0 ? x + w : x;
  var topH = h >= 0 ? y : y + h;
...
var notification = {
  ...
  show: function (type, message) {
    ...
    if (type == "success") {
      ...
      icon.src = chrome.runtime.getURL("") + "images/success.gif";
    }
