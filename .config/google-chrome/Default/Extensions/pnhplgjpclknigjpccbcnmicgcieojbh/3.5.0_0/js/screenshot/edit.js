...
function editor_style_callback(type, value) {
  switch (type) {
    ...
    case "redo":
      ...
      break;
    case "del":
      $('.single-btn[data-action="deleteSelected"]').toggleClass(
        "disabled",
        !value
      );
      break;
      ...
      if (action === "crop") {
        ...
        if (scale_sub != 5) {
          ...
          $(".zoom-tip")
            .addClass("active")
            .find(".zoom-level")
            .text(Number(scales[scale_sub]) * 100 + "%");
        }
      ...
      switch (type) {
        ...
        case "entire":
          ...
          function prepareNextCol() {
            if (++j > hlen - 1) return;
            if (j == hlen - 1)
              (sx = w - lastW), (sw = dw = editW - j * w), (dx = j * w);
  ...
  $("#launch-app")
    ...
    .click(function () {
      if (isAppInstalled) {
        ...
        chrome.tabs.create({
          url: "https://chrome.google.com/webstore/detail/awesome-screenshot-app/mfpiaehgjbbfednooihadalhehabhcjo"
        });
      }
    });
    ...
    .keydown(function (e) {
      ...
      if (id) {
        if (!$("body").hasClass("selected")) {
          ...
          if (id == "done" || "cancel") tool(id);
        }
        if (id != "undo") ctrl = false;
      }
    })
    .keyup(function (e) {
      switch (e.which) {
...
function i18n() {
  ...
  document.getElementById("save_button").lastChild.data =
...
function save() {
  ...
  $("#re-edit")
    .unbind()
    .text(chrome.i18n.getMessage("reEdit"))
    .click(function () {
      if (uploadFlag == true) {
  ...
  function prepareImage() {
    ...
    var base64 = $("#save-image")
      .attr("src")
      .split(",")[1]
      .replace(/\+/g, "%2b");
    var fname = tabtitle.replace(/[#$~!@%^&*();'"?><\[\]{}\|,:\/=+-]/g, " ");
    var ftype = $("#save-image")
      .attr("src")
      .split(",")[0]
      .split("/")[1]
      .split(";")[0];
...
function addMargin() {
  return;
  offsetX || (offsetY != 48 && offsetY != 88)
    ? $editArea.addClass("add-margin")
    : $editArea.removeClass("add-margin");
}
...
$(document).ready(function () {
  ...
  function print() {
    ...
    var imagediv =
      '<div style="margin:0 auto;text-align:center">' + printarea + "</div>";
