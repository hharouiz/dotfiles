...
Zotero.Inject = new function () {
  var _translate; // Used to display a different message for failing translations on pages
  // with site-access limits


  const siteAccessLimitsTranslators = new Set(["57a00950-f0d1-4b41-b6ba-44ff0fc30289" // GoogleScholar
  ]);
  this.sessionDetails = {};
  this.translators = [];
  /**
  ...
  this.init = async function (force) {
    ...
    this.sessionDetails = {}; // wrap this in try/catch so that errors will reach logError
  ...
  this.initTranslation = async function (document, sessionID, itemType) {
    ...
    if (sessionID) {
      ...
      translate.setHandler("select", function (obj, items, callback) {
        ...
        Zotero.Messaging.sendMessage("progressWindow.close", null); // If the handler returns a non-undefined value then it is passed
        ...
        (async function () {
          ...
          if (returnItems && !Zotero.Utilities.isEmpty(returnItems)) {
            let sessionID = this.sessionDetails.id; // Record how many items are being saved so that progress window can know
      ...
      translate.setHandler("itemSaving", function (obj, item) {
        itemsSaved += 1; // this relays an item from this tab to the top level of the window
  ...
  function determineAttachmentIcon(attachment) {
    ...
    return Zotero.ItemTypes.getImageSrc(contentType === "application/pdf" ? "attachment-pdf" : "attachment-snapshot");
  }
  ...
  this.loadReactComponents = async function (components = []) {
    ...
    if (typeof ReactDOM === "undefined" || typeof React === "undefined" || !React.useState) {
      toLoad = ['lib/react.js', 'lib/react-dom.js', 'lib/prop-types.js'];
    }
  ...
  this.confirm = async function (props) {
    await Zotero.initializedPromise; // Remove MV3 importConfirm hash from the history after displaying the prompt so that going back
  ...
  this.notify = new function () {
    ...
    return function (text, buttons, timeout, tabStatus) {
      ...
      }.bind(Zotero.Inject); // Sequentialize notification display
  ...
  this.firstUsePrompt = function () {
    ...
    return this.confirm({
      ...
      button2Text: "",
      message: Zotero.getString('firstRun_text1', [clientName, "https://www.zotero.org/support/adding_items_to_zotero"]) + '<br><br>' + Zotero.getString('firstRun_text2', [clientName, // TODO: Make download URL configurable (instead of just base URL + "download")
  ...
  this.firstSaveToServerPrompt = async function () {
    ...
    var result = await this.confirm({
      ...
      message: Zotero.getString('error_connection_save', [Zotero.getString('appConnector', clientName), clientName, ZOTERO_CONFIG.DOMAIN_NAME]) + '<br /><br />' + Zotero.Inject.getConnectionErrorTroubleshootingString()
    });
  ...
  this.getConnectionErrorTroubleshootingString = function () {
    ...
    return Zotero.getString('error_connection_downloadOrTroubleshoot', [downloadLink, clientName, troubleshootLink]);
  };
  /**
  ...
  this.translate = async function (translatorID, options = {}) {
    ...
    if (!result) return;
    var translator = this.translators.find(t => t.translatorID == translatorID); // In some cases, we just reopen the popup instead of saving again

    if (this.sessionDetails.id // Same page (no history push)
    && document.location.href == this.sessionDetails.url // Same translator
    && translatorID == this.sessionDetails.translatorID // Not a multiple page
    && translator.itemType != 'multiple' // Not "Create Zotero Item and Note from Selection"
    && !options.note // Not from the context menu, which always triggers a resave
    && !options.resave) {
    ...
    Zotero.Messaging.sendMessage("progressWindow.show", [sessionID, null, false, // If we're likely to show the Select Items window, delay the opening of the
    ...
    while (true) {
      ...
      try {
        ...
        let errorMessage = e.toString(); // TEMP: Remove once client switches automatically (added in 5.0.46)
        ...
        if (translator.itemType != 'multiple') {
          ...
          this.sessionDetails = {}; // We delay opening the progressWindow for multiple items so we don't have to flash it
  ...
  this.saveAsWebpage = async function (args) {
    ...
    if (!result) return;
    var translatorID = 'webpage' + (options.snapshot ? 'WithSnapshot' : ''); // Reopen if popup instead of resaving

    if (this.sessionDetails.id // Same page (no history push)
    && document.location.href == this.sessionDetails.url // Same translator
    && translatorID == this.sessionDetails.translatorID // Not from the context menu, which always triggers a resave
    && !options.resave) {
      let sessionID = this.sessionDetails.id;
      Zotero.Messaging.sendMessage("progressWindow.show", [sessionID]);
      return;
...
const isTestPage = Zotero.isBrowserExt && window.location.href.startsWith(browser.runtime.getURL('test')); // don't try to scrape on hidden frames
...
if (!isHiddenIFrame) {
  var doInject = async function () {
    ...
    if (!isWeb && !isTestPage) return; // add listener for translate message from extension
