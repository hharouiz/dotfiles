...
(() => {
  ...
  // shared/js/newtab/schema.js
  var import_constants = __toESM(require_constants());
  var { events } = import_constants.default.trackerStats;
  var dataFormatSchema = z.object({
  ...
  var disconnectMessage = z.object({
    messageType: z.literal(events.outgoing.newTabPage_disconnect)
  });
  var dataMessage = z.object({
    messageType: z.literal(events.outgoing.newTabPage_data),
    options: dataFormatSchema
  });
  var outgoing = z.discriminatedUnion("messageType", [
    dataMessage,
    disconnectMessage
  ]);
  var heartbeatMessage = z.object({
    messageType: z.literal(events.incoming.newTabPage_heartbeat)
  });
  var incoming = z.discriminatedUnion("messageType", [
    heartbeatMessage
  ]);

  // shared/js/newtab/newtab.js
  var { events: events2, allowedOrigin, clientPortName } = import_constants2.default.trackerStats;
  var receivedMessageFromAllowedOrigin = false;
  function connect() {
    const port2 = chrome.runtime.connect({ name: clientPortName });
    port2.onMessage.addListener((msg) => {
      if (msg.messageType in events2.outgoing) {
        sendToNewTabPage(msg);
      }
    });
    port2.onDisconnect.addListener((msg) => {
      console.log("\u{1F6A8} PORT onDisconnect", msg);
    });
    return port2;
  }
  window.addEventListener("message", (e) => {
    if (!e.isTrusted || e.origin !== import_constants2.default.trackerStats.allowedOrigin) {
    ...
    if (!(e.data.messageType in events2.incoming)) {
  ...
  function sendToChromeRuntime(msg) {
    if (!receivedMessageFromAllowedOrigin)
      return;
    try {
      if (!port) {
        port = connect();
      ...
      sendToNewTabPage({ messageType: events2.outgoing.newTabPage_disconnect });
    }
  }
})();
