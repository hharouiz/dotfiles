...
const PopupModule = function (fvdSynchronizer) {
  fvdSynchronizer.Popup = new (function () {
    const DISPLAY_RATE_US_AFTER = 3600 * 24 * 1;

    var observerHolder = fvdSynchronizer.Observer.createHolder();
    fvdSynchronizer.addEventListener(EventType.UNLOAD, () => {
      if (!observerHolder || typeof observerHolder.cleanup !== 'function') {
        console.info('observerHolder.cleanup is not a function', observerHolder);
      } else {
    ...
    this.refreshSyncState_Speeddial = function () {
      ...
      if (progress.state) {
        document
          .getElementById('speedDialProgressState_' + progress.state)
          .setAttribute('state', 'progress');
      }
      ...
      if (progress.action == 'special_actions') {
        text = text.replace(
          '%progress%',
          progress.specialActionsCurrent + '/' + progress.specialActionsCount
        );
      }
    ...
    function refreshDriversSyncing() {
      ...
      if (
        fvdSynchronizer.Driver.Bookmarks.canSync() ||
        fvdSynchronizer.Driver.Bookmarks.isSyncNow()
      ) {
    ...
    chrome.runtime.onMessage.addListener(function (request) {
      if (request.subject == 'speedDialConnected') {
        initSyncDriversList();
      } else if (request.subject == 'mainSyncStateChange') {
        //refreshMainSyncState();
      } else if (request.subject == 'syncError') {
    ...
    fvdSynchronizer.addEventListener(
      ...
      function () {
        if (!fvdSynchronizer.Driver.Bookmarks.supportsLargeSync()) {
          ...
          document.querySelector('#bookmarks-older-chrome-message a').addEventListener(
            'click',
            function (e) {
              e.preventDefault();
        ...
        document.querySelector('#warning-unsorted a').addEventListener(
          'click',
          function (e) {
            var locale = chrome.i18n.getMessage('extension_locale');
            chrome.tabs.create({
              url:
                'https://everhelper.pro/info/sync-unsorted.php?locale=' + encodeURIComponent(locale),
              active: true,
            });
            e.preventDefault();
          },
        ...
        fvdSynchronizer.Utils.Async.chain([
          ...
          function () {
            ...
            if (activityState != 'logged') {
              ...
              mustLoggedMessage
                .querySelector('a.login')
                .setAttribute('href', chrome.runtime.getURL('options.html#login'));
              mustLoggedMessage
                .querySelector('a.register')
                .setAttribute('href', chrome.runtime.getURL('options.html#register'));
            } else {
            ...
            document.getElementById('buttonSyncSpeedDial').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('buttonSyncBookmarks').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('syncCancelButton').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('syncSDCancelButton').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('syncCancelButtonBookmarks').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('buttonOpenOptions').addEventListener(
              'click',
              function () {
            ...
            document.getElementById('buttonReportBug').addEventListener(
              'click',
              function () {
                chrome.tabs.create({
            ...
            document.getElementById('buttonLogout').addEventListener(
              'click',
              function () {
            ...
            document.querySelector('.bottomMenu .goToEverhelper').addEventListener(
              'click',
              function () {
