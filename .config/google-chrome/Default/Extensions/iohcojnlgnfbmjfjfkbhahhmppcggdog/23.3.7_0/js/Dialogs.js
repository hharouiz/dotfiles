...
const DialogsModule = function (fvdSynchronizer) {
  ...
  Dialogs.prototype = {
    ...
    fireOpenedAlerts: function () {
      fvdSynchronizer.Dialogs._alertWindowsIds.forEach(function (id) {
        ...
        function () {
          ...
          const left =
            typeof window !== 'object'
              ? 0
              : Math.round(window.screen.width / 2 - INITIAL_WIDTH / 2);
          const top =
            typeof window !== 'object' ? 0 : Math.round(window.screen.height / 2 - INITIAL_HEIGHT / 2);
          chrome.windows.create(
            ...
            function (w) {
              fvdSynchronizer.Dialogs._alertWindowsIds.push(w.id);
            }
          );
        },
      ]);
    ...
    alertWindowOpened: function (callback) {
      fvdSynchronizer.Utils.Async.arrayProcess(
        fvdSynchronizer.Dialogs._alertWindowsIds,
        function (windowId, arrayProcessCallback) {
      ...
      fvdSynchronizer.Utils.Async.chain([
        ...
        function () {
          ...
          if (type == 'needSyncSoftware') {
            ...
            INITIAL_HEIGHT = 170;
            if (
              _b(fvdSynchronizer.Prefs.get('dont_display_alert_sync_software')) &&
              !params.force
            ) {
              return;
            }
          ...
          if (params.btnText) {
            url += '&btn_text=' + encodeURIComponent(params.btnText);
          }
          if (params.btnLink) {
            url += '&btn_link=' + encodeURIComponent(params.btnLink);
          }
          chrome.windows.create(
            {
              url,
    ...
    alertCheck: function (title, text, cbText, cbInitState, callback, params) {
      ...
      var dlg = new Dialog({
        ...
        width: params.width,
        title: title,
        content: document.getElementById('prototype_dialogAlertCheck').innerHTML,
        buttons: btns,
        onShow: function () {
    ...
    syncProgressSimple: function (syncDriver, syncAction, callback, additional) {
        ...
        btns[_('dlg_sync_progress_cancel')] = function () {
          if (syncInProgress) {
            fvdSynchronizer.Dialogs.confirm(
              _('dlg_confirm_abort_sync_title'),
              _('dlg_confirm_abort_sync_title'),
              function (r) {
                if (r) {
      ...
      function endCallback(error) {
        ...
        var manualSyncProgressMessage = document.querySelector(
          '.dialog .syncProgressDialog  .manualSyncInProcess'
        );
        if (manualSyncProgressMessage) {
          manualSyncProgressMessage.setAttribute('hidden', true);
        }
        var manualSyncSuccessMessage = document.querySelector(
          '.dialog .syncProgressDialog  .manualSyncSuccess'
        );
        if (error) {
        } else if (manualSyncSuccessMessage.hasAttribute('display')) {
        ...
        if (error != 0) {
          if (error == fvdSynchronizer.Errors.ERROR_NEED_ADDITIONAL_SOFTWARE) {
            try {
              var actionsContainer = dlg.container
                .querySelector('.syncProgressDialog')
                .parentNode.parentNode.querySelector('.dialog-actions');
          ...
          setTimeout(function () {
            if (error == fvdSynchronizer.Errors.ERROR_QUOTA_EXCEED) {
              fvdSynchronizer.Dialogs.alert(
                _('dlg_sync_quota_exceed_title'),
                _('dlg_sync_quota_exceed_text')
              );
            } else if (error == fvdSynchronizer.Errors.ERROR_ALREADY_LOCKED) {
              fvdSynchronizer.Dialogs.alert(
                _('popup_alert_sync_already_active_title'),
                _('popup_alert_sync_already_active')
              );
            }
          }, 0);
        } else {
      ...
      } else if (syncAction == 'restoreBackup') {
        title = _('dlg_sync_progress_title_restore');
      }
      var dialogHTML = document.getElementById('prototype_dialogSyncProgressSimple').innerHTML;
      dialogHTML = dialogHTML.replace(
        '{{locale}}',
        chrome.i18n.getMessage('extension_locale'),
        dialogHTML
      );
      var dlg = new Dialog({
        width: 400,
        ...
        onShow: function (_dlg) {
          ...
          if (syncAction == 'makeBackup') {
            ...
            var _progressCheckInterval = setInterval(function () {
              ...
              _dlg.container.querySelector('.simpleProgressItems .value').textContent =
                p.current + '/' + p.max;
            }, 500);
    ...
    syncProgress: function (syncType, callback) {
            ...
            function (r) {
              if (r) {
                var stateText = document.querySelector(
                  '.dialog .syncProgressDialog .syncProgressLine'
                );
      ...
      function successInProgressStep() {
        var elem = document.querySelector('.dialog .syncProgressDialog div[state=progress]');
        if (elem) {
      ...
      function changeStateCallback(type, data) {
        if (data) {
          if (typeof data.groupsCount != 'undefined') {
            document.querySelector('.dialog #dlgSyncProgress_syncedCountGroups').textContent =
              data.groupsCount;
          }
          if (typeof data.dialsCount != 'undefined') {
            document.querySelector('.dialog #dlgSyncProgress_syncedCountDials').textContent =
              data.dialsCount;
          }
        }
        ...
        document.querySelector('.dialog #' + elem).setAttribute('state', 'progress');
      }
      function errorAll() {
        var elements = document.querySelectorAll(
          '.dialog .syncProgressDialog  .progressElements > div'
        );
      ...
      function endCallback(error) {
        ...
        syncInProgress = false;
        successInProgressStep();
        var stateText = document.querySelector('.dialog .syncProgressDialog .syncProgressLine');
        if (error != 0) {
          stateText.setAttribute('state', 'fail');
          stateText.textContent = _('dlg_sync_progress_sync_failed');
          errorAll();
      ...
      var dlg = new Dialog({
        ...
        content: document.getElementById('prototype_dialogSyncProgress').innerHTML,
        buttons: btns,
        onShow: function () {
          switch (syncType) {
            ...
            case 'sdOverwriteServerData':
              fvdSynchronizer.Driver.Speeddial.overwriteServerData(
                endCallback,
                changeStateCallback
              );
              break;
    ...
    addGroup: function (groupId, dialogParams) {
          ...
          fvdSpeedDial.Storage.groupExists(
            ...
            function (exists) {
              if (exists) {
                ...
                if (groupId) {
                  fvdSpeedDial.Storage.groupUpdate(
                    groupId,
                    {
                      name: name,
                    },
      ...
      var dlg = new Dialog({
        ...
        content: document.getElementById('prototype_dialogAddGroup').innerHTML,
        buttons: btns,
  ...
  fvdSynchronizer.Dialogs = new Dialogs();
};
