const ServerSyncModule = function (fvdSynchronizer) {
  var IS_LOCAL = false;
  ...
  var Sync = function (fvdSynchronizer) {
    var self = this;
    ...
    this.setActivityState = function (state) {
      ...
      if (state == 'not_logged' && typeof window === 'object') {
    ...
    this.loginInAdminPanel = function (callback) {
      ...
      fvdSynchronizer.Utils.Async.chain([
        ...
        function (chainCallback) {
          fvdSynchronizer.Server.Connection.get(
            SYNC_ADMIN_URL + '/common/i_am_logged',
            function (response) {
              if (!response) {
                return callback(false);
              }
        ...
        function () {
          fvdSynchronizer.Server.Connection.post(
            SYNC_ADMIN_URL + '/login',
            JSON.stringify({
    ...
    this.userInfo = function (cb) {
      const savedUserInfo = fvdSynchronizer.Prefs.get('userInfo');
      if (savedUserInfo) {
        cb(null, savedUserInfo);
        return;
      }
      ...
      this.authorizedMessage(
        ...
        (err, data) => {
          if (!err && data) {
            fvdSynchronizer.Prefs.set('userInfo', data);
          }
    ...
    this.removeAuthCookie = function () {
      ...
      new Request('https://everhelper.pro/spec/clear_auth_cookie.php', params);
      /*
  ...
  fvdSynchronizer.Server.Sync = new Sync(fvdSynchronizer);
...
export default ServerSyncModule;
