...
import { switchTabs } from './init.js';

const OptionsModule = function (fvdSynchronizer) {
  chrome.tabs.query(
    {
  ...
  fvdSynchronizer.Options = new (function (fvdSynchronizer) {
    var self = this;
    ...
    function removeFieldError(fieldId) {
      document.getElementById(fieldId).className = document
        .getElementById(fieldId)
        .className.replace('errorField', '');
    }
      ...
      fvdSynchronizer.Utils.Async.chain([
        function (next) {
          ...
          fvdSynchronizer.Dialogs.confirm(
            _('dlg_speeddial_confirm_overwrite_server_data_title'),
            _('dlg_speeddial_confirm_overwrite_server_data_text'),
            function (result) {
              if (!result) {
                return;
              }
    ...
    this.bookmarksMergeLocalAndServerData = function () {
      fvdSynchronizer.Utils.Async.chain([
        ...
        function () {
          fvdSynchronizer.Dialogs.syncProgressSimple(
            'Bookmarks',
      ...
      fvdSynchronizer.Utils.Async.chain([
        function (next) {
          ...
          fvdSynchronizer.Dialogs.confirm(
            ...
            _('dlg_bookmarks_confirm_overwrite_server_data_text'),
            function (result) {
              if (!result) {
                return;
              }
        ...
        function () {
          fvdSynchronizer.Utils.Async.chain([
            ...
            function () {
              var dlg = fvdSynchronizer.Dialogs.syncProgressSimple(
                'Bookmarks',
                'overwriteServerData',
                function () {
                  self.refreshInitialSync();
      ...
      fvdSynchronizer.Driver.Bookmarks.Backup.listBackups(function (backups) {
        ...
        backups.forEach(function (backup, index) {
          ...
          button.addEventListener(
            ...
            function () {
              fvdSynchronizer.Dialogs.confirm(
                _('confirm_really_restore_bookmarks_title'),
                _('confirm_really_restore_bookmarks_text'),
                function (r) {
                  if (r) {
      ...
      fvdSynchronizer.Driver.Speeddial.Backup.listBackups(function (backups) {
        ...
        backups.forEach(function (backup, index) {
          ...
          button.addEventListener(
            ...
            function () {
              fvdSynchronizer.Dialogs.confirm(
                _('confirm_really_restore_sd_title'),
                _('confirm_really_restore_sd_text'),
                function (r) {
                  if (r) {
      ...
      fvdSynchronizer.Driver.Bookmarks.Backup.hasBackup(function (has) {
        ...
        if (lastDate) {
          try {
            ...
            document.getElementById('bookmarksLastBackupDate').textContent =
    ...
    this.setTab = function (tab) {
      ...
      const main_tab = String(tab).split('_').shift();
      if (!switchTabs.hasOwnProperty(main_tab)) {
        main_tab = 'account';
      }
      fvdSynchronizer.Prefs.set('last_settings_tab_index', tab);
      switchTabs[main_tab](tab, fvdSynchronizer);
    };
    ...
    fvdSynchronizer.addEventListener(
      ...
      function () {
        ...
        document.querySelector('#bookmarks-older-chrome-message .inner a').addEventListener(
          'click',
          function () {
            chrome.tabs.create({
              url: 'chrome://chrome/',
        ...
        document.getElementById('buttonpremiumForShareButton').addEventListener(
          'click',
          function () {
        ...
        fvdSynchronizer.Utils.Async.chain([
          ...
          function () {
            ...
            document.getElementById('enableAutoBackup').checked = _b(
              fvdSynchronizer.Prefs.get('autobackup.enabled')
            );
            ...
            for (var i = 0; i != toolTipElems.length; i++) {
              (function (el) {
                el.addEventListener(
                  'click',
                  function (event) {
                    fvdSynchronizer.ToolTip.display(
                      el,
                      document.getElementById(el.getAttribute('tooltip')).innerHTML,
                      event
                    );
                  },
                  false
            ...
            var isBookmarksInitialization =
              fvdSynchronizer.Driver.Bookmarks.initializationInProgress();
            ...
            if (isBookmarksInitialization) {
              ...
              function _bintervalFunc() {
                ...
                document.getElementById('bookmarksInitializationProgress').textContent =
                  p.current + ' / ' + p.total;
              }
            ...
            fvdSynchronizer.Observer.registerCallback(
              'bookmarksInitializationCompleted',
              function () {
            ...
            events.forEach(function (event) {
              const element = document.getElementById(event.elem);
              if (element) {
            ...
            for (var i = 0; i != containers.length; i++) {
              (function (container) {
                var messageContainer = document.getElementById(
                  container.getAttribute('helpmessageto')
                );
    ...
    var _observerStruct = {
      'syncQuotaExceed': function () {
        var msg = document.querySelector('.dialog .syncProgressDialog .syncQuotaExceed');
        if (msg) {
