...
import { _r, _b, _isb, SimpleBench } from '../Utils.js';
import { Log } from '../log.js';

// let _log = new Log();

const DriverBookmarksModule = function (fvdSynchronizer) {
  /***********************************/
  /** Main Bookmarks Driver Interface*/

  const Bookmarks = function (fvdSynchronizer) {
    let _log;
    ...
    function initialActions(callback) {
      ...
      fvdSynchronizer.Utils.Async.chain([
        function (chainCallback) {
          ...
          var ids = fvdSynchronizer.bookmarks.database.getAllIds();
          var _processed = 0;
          if (!ids.length) {
          ...
          ids.forEach(function (id) {
            fvdSynchronizer.bookmarks.manager.bookmarkExistsByRealId(id, function (exists) {
              _processed++;
              if (!exists) {
                fvdSynchronizer.bookmarks.database.removeGuid(id);
              }
              _end();
            });
          });
        ...
        function (chainCallback) {
          ...
          fvdSynchronizer.bookmarks.manager.createGuidsForAll(
            function () {
    ...
    function saveListIfFailTryPlugin(bookmarks, callback, params) {
      ...
      fvdSynchronizer.bookmarks.manager.saveList(
        bookmarks,
        function (result) {
          callback(result);
        },
    ...
    function rollbackTransaction(transId, callback) {
      fvdSynchronizer.bookmarks.database.rollbackMassGuidChange();
      callback();
    }
    ...
    function normalizeServerBookmark(bookmark) {
      if (bookmark.type == 'bookmark') {
        if (!bookmark.url) {
          ...
          } else if (bookmark.url.indexOf('[') != -1 || bookmark.url.indexOf(']') != -1) {
          ...
          fvdSynchronizer.Utils.Async.chain([
                  ...
                  function (id, apc) {
                    chrome.bookmarks.getSubTree(id, function (bookmarks) {
                      ...
                      bookmarks[0].children.forEach(function (b) {
                        var global_id = fvdSynchronizer.bookmarks.manager.getGlobalId(b.id);
            ...
            function () {
              ...
              if (global_ids.length > 0) {
                fvdSynchronizer.Utils.Async.arrayProcess(
                  global_ids,
                  function (global_id, arrayProcessCallback) {
                    var saveInfo = uploadChanges[global_id];
                    fvdSynchronizer.bookmarks.manager.getBookmarkById(
                      global_id,
                      function (bookmark) {
                        if (bookmark) {
                          if (saveInfo == 'full') {
    ...
    function overwriteServerData(callback) {
      fvdSynchronizer.Utils.Async.chain([
        ...
        function (chainCallback) {
          fvdSynchronizer.bookmarks.manager.getAllBookmarks(function (clientBookmarks) {
    ...
    function overwriteLocalData(callback) {
      ...
      fvdSynchronizer.bookmarks.database.startMassGuidsChange();
      fvdSynchronizer.Utils.Async.chain([
        function (chainCallback) {
          // get current bookmarks count to remove
          log.profile.start('fvdSynchronizer.bookmarks.manager.getAllBookmarks');
          fvdSynchronizer.bookmarks.manager.getAllBookmarks(null, function (bookmarks) {
            log.profile.end('fvdSynchronizer.bookmarks.manager.getAllBookmarks');
            ...
            if (error) {
              ...
              fvdSynchronizer.Utils.Async.chain([
                function (next) {
                  ...
                  fvdSynchronizer.Utils.Async.arrayProcess(
                    toRemove,
                    function (clientBookmark, apNext) {
                      fvdSynchronizer.bookmarks.manager.remove(
                        clientBookmark.global_id,
                        function () {
                          apNext();
                        },
                        {},
                    ...
                    function () {
                      // cleanup guids
                      try {
                        var allGuids = fvdSynchronizer.bookmarks.guidsStorage.getAllGuids();
                        for (var i = 0; i != allGuids.length; i++) {
                          if (allGuids[i] != 'unsorted') {
                            if (!serverGuids[allGuids[i]]) {
                              fvdSynchronizer.bookmarks.guidsStorage.removeGuid(allGuids[i]);
                            }
                          }
                        }
                      } catch (ex) {
                ...
                function () {
                  fvdSynchronizer.bookmarks.virtualBookmarks.empty();
                  saveListIfFailTryPlugin(
                    data.bookmarks,
                    function (result, newInterfaceParams) {
                      if (!result) {
        ...
        function () {
          fvdSynchronizer.bookmarks.database.applyMassGuidsChange();
          callback(0);
        },
      ]);
    }
    ...
    function mergeLocalAndServerData(callback) {
      ...
      fvdSynchronizer.Utils.Async.chain([
        ...
        function (chainCallback) {
          ...
          fvdSynchronizer.bookmarks.manager.getAllBookmarks(function (_bookmarks) {
          ...
          function processItems(parentIds) {
            ...
            clientItems.forEach(function (clientItem) {
              ...
              if (collisedServerItem) {
                if (
                  !(clientItem.global_id in foundCollisionsFor) &&
                  !(collisedServerItem.global_id in foundCollisionsFor)
                ) {
        ...
        function (chainCallback) {
          ...
          collisions.forEach(function (collision) {
            ...
            if (collision.serverItem.global_id != collision.clientItem.global_id) {
              translateBookmarksGuids[collision.clientItem.global_id] =
                collision.serverItem.global_id;
              // change global id to server global id
              fvdSynchronizer.bookmarks.manager.changeGlobalId(
                collision.clientItem.global_id,
                collision.serverItem.global_id
              );
            }
          });
          ...
          // save new bookmarks
          console.log(
            'Merge step: store bookmarks interface:',
            storeToClientInterfaceParams,
            toClientBookmarks.length
          );
          saveListIfFailTryPlugin(
            toClientBookmarks,
            function (result, newInterfaceParams) {
    ...
    function applyServerUpdates(params, callback) {
      ...
      getBookmarksList(params, function (error, data) {
        if (error != 0) {
          callback(error);
        } else {
          var changeData = getToSyncData(),
          ...
          fvdSynchronizer.bookmarks.database.startMassGuidsChange();
          allGuids = fvdSynchronizer.bookmarks.manager.getAllGuids();
          // remove bookmarks
          ...
          fvdSynchronizer.bookmarks.virtualBookmarks.removeNotInList(data.bookmarkIds);

          fvdSynchronizer.Utils.Async.chain([
            function (chainCallback) {
              fvdSynchronizer.Utils.Async.arrayProcess(
                toRemove,
                function (clientId, arrayProcessCallback) {
                  fvdSynchronizer.bookmarks.manager.remove(
                    clientId,
                    function () {
                      arrayProcessCallback();
    ...
    this.removeSyncData = function (...args) {
      removeSyncData.apply(self, ...args);
    };

    this.syncData = function (...args) {
      syncData.apply(self, ...args);
    };
    ...
    this.displayChromeQuotaExceedMessage = function () {
      fvdSynchronizer.Dialogs.alertWindow(
        _('dlg_alert_sync_failed_chrome_quota_exceed_title'),
        _('dlg_alert_sync_failed_chrome_quota_exceed_text')
      );
    };
    ...
    this.Backup = new (function () {
      ...
      const AUTOBACKUP_CHECK_INTERVAL = 60 * 1000;
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            ...
            function (dir, apc) {
              ...
              fvdSynchronizer.FileSystem.read(
                'backups/bookmarks/' + dir + '/info.json',
                function (r, contents) {
                  if (!r) {
                    console.error(
                      'Fail read file ' + 'backups/bookmarks/' + dir + '/info.json, ignore',
                      r
                    );
                    return apc();
                  }
      ...
      this.make = function (callback) {
        ...
        fvdSynchronizer.Utils.Async.chain([
          function (chainCallback) {
            fvdSynchronizer.bookmarks.manager.getAllBookmarks(false, function (_bookmarks) {
      ...
      this._restore = function (dir, callback) {
        ...
        fvdSynchronizer.FileSystem.read(
          'backups/bookmarks/' + dir + '/data.json',
          function (result, data) {
            ...
            var currentBookmarksCount = fvdSynchronizer.bookmarks.database.getCountStoredGuids();
            ...
            fvdSynchronizer.Utils.Async.chain([
              function () {
                try {
                  ...
                  fvdSynchronizer.bookmarks.manager.removeAllBookmarks(
                    function () {
                      ...
                      bookmarks.forEach(function (b) {
                        fvdSynchronizer.Driver.Bookmarks.syncData(
                          ['changedIds', 'newIds'],
                          b.global_id,
                          false
                        );
  ...
  fvdSynchronizer.Driver.Bookmarks = new Bookmarks(fvdSynchronizer);
};

export default DriverBookmarksModule;
