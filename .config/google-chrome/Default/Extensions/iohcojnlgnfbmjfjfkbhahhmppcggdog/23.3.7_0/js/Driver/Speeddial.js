...
const DriverSpeeddialModule = function (fvdSynchronizer) {
  var Errors = fvdSynchronizer.Errors;

  const Speeddial = function (fvdSynchronizer) {
    ...
    this.onLogin = function(userInfo) {
      messageToSD({ action: 'sync:login', data: { user: userInfo } });
    }

    this.onLogout = function () {
      fvdSynchronizer.Prefs.set('userInfo', null);
      messageToSD({ action: 'sync:logout' });
    };
    ...
    this.startMainSync = function (options, callback) {
      ...
      acquireSyncLock(function (error) {
        if (error !== 0) {
          ...
          var fvdSpeedDialLastUpdateTime = {
            dials: 1,
            groups: 1,
          }
          ...
          fvdSynchronizer.Utils.Async.chain([
            ...
            function (chainCallback) {
              getSyncDialsAndGroupsLastUpdateTime(function(result) {
                if (result) {
                  fvdSpeedDialLastUpdateTime = result;
                }
                chainCallback();
              });
            },
            ...
            function (chainCallback) {
              ...
              if (fvdSynchronizer.Prefs.get('speeddial.first_sync_after') !== 'none') {
                ...
                fvdSynchronizer.Driver.Speeddial.mergeLocalAndServerData(
                  function (error) {
                    setMainSyncState('nosync');
                ...
                fvdSynchronizer.Driver.Speeddial.hasUpdates(function (error, has, updateInfo) {
                  ...
                  fvdSynchronizer.Utils.Async.chain([
                    function (chainCallback) {
                      ...
                      self.applyServerUpdates(
                        // updateInfo.localLastUpdateTimeGroups,
                        // updateInfo.localLastUpdateTimeDials,
                        fvdSpeedDialLastUpdateTime.groups,
                        fvdSpeedDialLastUpdateTime.dials,
                        function (error) {
                          if (error) {
                            syncError = error;
                          }
                          ...
                          if (error === 0) {
                            messageToSD({ 
                              ...
                              data: { 
                                key: 'syncLastUpdateTime',
                                value: {
                                  dials: getDialsLastUpdateTime() || 1,
                                  groups: getGroupsLastUpdateTime() || 1
                                }
                              } 
                            })
                          }
            ...
            function (chainCallback) {
              ...
              fvdSynchronizer.Utils.Async.arrayProcess(
                specialActions,
                function (specialAction, arrayProcessCallback) {
                  nullMainSyncProgress();
            ...
            function (chainCallback) {
              if (syncError != 0) {
                if (syncError == fvdSynchronizer.Errors.ERROR_COUNT_ITEMS_QUOTA_EXCEED) {
                  ...
                  //   _('dlg_alert_sync_failed_title'),
                  //   _('dlg_alert_sync_failed_text')
                  // );
                }
    ...
    this.gMessageToSD = function (message, callback) {
    ...
    function getSyncDialsAndGroupsLastUpdateTime(callback) {
      messageToSD({ action: 'localStorage.get', data: { key: 'syncLastUpdateTime' } }, function (response) {
    ...
    //   fvdSynchronizer._pendingSDRequests.push({
    //     id: id,
    //     callback: callback,
    ...
    //   for (var i = 0; i != fvdSynchronizer._pendingSDRequests.length; i++) {
    //     if (id == fvdSynchronizer._pendingSDRequests[i].id) {
    //       request = fvdSynchronizer._pendingSDRequests[i];
    //       break;
    //     }
    //   }
    ...
    //   var index = null;
    //   for (var i = 0; i != fvdSynchronizer._pendingSDRequests.length; i++) {
    //     if (fvdSynchronizer._pendingSDRequests[i] == id) {
    //       index = i;
    //       break;
    //     }
    //   }
    ...
    //   if (index !== null) {
    //     fvdSynchronizer._pendingSDRequests.splice(index, 1);
    //   }
    // }
    ...
    //   fvdSynchronizer._pendingSDRequests.forEach(function (request) {
    ...
    function mapDialsLocalToServer(localDials, extra) {
      ...
      localDials.forEach(function (localDial) {
        ...
        var isManuallyCroppedPreview =
          localDial.get_screen_method == 'manual' && localDial.thumb_source_type == 'screen';
        if (localDial.thumb_source_type == 'local_file' || isManuallyCroppedPreview) {
          if (
            typeof localDial.need_sync_screen == 'undefined' ||
            localDial.need_sync_screen ||
            extra.forceStorePic
          ) {
            try {
      ...
      serverDials.forEach(function (serverDial) {
        ...
        if (
          (localDial.thumb_source_type == 'screen' ||
            localDial.thumb_source_type == 'local_file') &&
      ...
      fvdSynchronizer.Utils.Async.chain([
        function (chainCallback) {
          ...
          getGroupsFromServer(currentLastUpdateTime, function (error, result) {
            if (error != 0) {
              ...
              try {
                groups = result.groups.filter((serverGroup) => !toSyncData.deleteGroups.includes(serverGroup.global_id));
                      ...
                      messageToSD(
                        ...
                        function (response) {
                          ...
                          if (removeInfo.count > 0) {
                            ...
                            needFixDialsPositionsGroupsIds = needFixDialsPositionsGroupsIds.concat(
                              removeInfo.removedFromGroups
                            );
                          }
        ...
        function (chainCallback) {
          ...
          if (needFixDialsPositionsGroupsIds.length > 0) {
            needFixDialsPositionsGroupsIds = fvdSynchronizer.Utils.arrayUnique(
              needFixDialsPositionsGroupsIds
            );

            fvdSynchronizer.Utils.Async.arrayProcess(
              needFixDialsPositionsGroupsIds,
              function (groupId, arrayProcessCallback) {
                if (groupId) {
                  messageToSD(
                    {
    ...
    function putDialsListOnserver(dials, callback, extra) {
      ...
      fvdSynchronizer.Utils.Async.chain([
              ...
              if (dial._previewContent && typeof dial._previewContent === 'string') {
                fvdSynchronizer.Utils.Async.chain([
                  ...
                  function () {
                    ...
                    let blob;
                    try {
                      blob = fvdSynchronizer.Utils.dataURItoBlob(dial._previewContent);
                    } catch (ex) {
                      console.warn(ex, dial);
                      return apCallback();
                    }
                    fvdSynchronizer.Server.Sync.preUploadFile(
                      ...
                      function (error, data) {
                        ...
                        if (!data || !data.files) {
                          return callback(
                            fvdSynchronizer.Errors.ERROR_STORAGE_ENGINE_RETURNS_ERROR
                          );
                        }
        ...
        function (chainCallback) {
          ...
          let nfGroupsGlobalIds = putDialsServerResponse.extra.notFoundGroups.map(
            (group) => group.global_id
          );
            ...
            fvdSynchronizer.Utils.Async.chain([
              ...
              () => {
                ...
                const request = new Request(
                  'https://everhelper.pro/spec/nogroups_put_dials_dbg.php',
                  params
                );
                fetch(request)
                  .then((response) => {
    ...
    function mergeLocalAndServerData(params, callback, stateCallback) {
      ...
      fvdSynchronizer.Utils.Async.chain([
          ...
          function clientDialInList(dial, list) {
            for (var i = 0; i != list.length; i++) {
              if (fvdSynchronizer.Utils.isUrlsEqual(list[i].url, dial.url)) {
                ...
                if (
                  groupGlobalId == list[i].group_global_id &&
                  dialsToOverrideIds.indexOf(list[i].global_id) == -1
                ) {
        ...
        function (chainCallback) {
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            collisedGroups,
            function (collision, arrayProcessCallback) {
              messageToSD(
                {
        ...
        function (chainCallback) {
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            newGroups,
            function (newGroup, arrayProcessCallback) {
              messageToSD(
                {
                  action: 'saveGroup',
                  group: newGroup,
        ...
        function (chainCallback) {
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            newDials,
            function (newDial, arrayProcessCallback) {
        ...
        function () {
          localDatabaseAffected =
            newDials.length || newGroups.length || collisedGroups.length || collisedDials.length;
    ...
    function getTemporaryStoredSyncData() {
      let tempData = fvdSynchronizer.localStorage.getItem('speeddial.temporary_sync_data');
      if (!tempData) {
        tempData = [];
      } else {
        try {
    ...
    function storeSyncDataTemporary(category, data) {
      ...
      fvdSynchronizer.localStorage.setItem(
        'speeddial.temporary_sync_data',
    ...
    this.restoreTemporaryStoredSyncData = function () {
      ...
      fvdSynchronizer.localStorage.setItem('speeddial.temporary_sync_data', '[]');
    };
      ...
      fvdSynchronizer.Utils.Async.cc(function (next) {
        ...
        messageToSD(message, function (r) {
          ...
          if (typeof list === undefined) {
            console.warn('r.list is', typeof list, r);
            next();
          }
          console.log('got sub request dials', list.length, list);
          for (var i = 0; i != list.length; i++) {
            var dial = list[i];
    ...
    function messageToSD(message, callback) {
      console.info('messageToSD', { ...message });
      connectToSpeeddial();

      messageTries++;

      if (callback) {
        message.requestId = fvdSynchronizer.addPendingRequest(callback);
      }

      if (port) {
        port.postMessage(message);
        messageTries = 0;
      } else if (messageTries === 4) {
        console.log('Extension not found. syncData stoped.');
        return;
      } else {
        setTimeout(() => messageToSD(message, callback), 100);
      }
    ...
    function _setAllow(value, apiv) {
      allow = value;
      apiVersion = apiv;
      fvdSynchronizer.Observer.fireEvent('syncDriverAllowedStateChanged', ['SpeedDial']);
    }
    ...
    function portMessageProcess(message) {
      ...
      switch (message.action) {
          ...
          switch (message.type) {
            case 'main':
              fvdSynchronizer.Server.Sync.activityState(function (state) {
                if (state != 'logged') {
                  chrome.tabs.create({
                    url: chrome.runtime.getURL('options.html#login'),
                    active: true,
                  });
                  ...
                  chrome.tabs.create({
                    url: chrome.runtime.getURL('options.html#sync'),
                    active: true,
                  });
                }
        ...
        case '_response':
          ...
          if (message.requestId) {
            var request = fvdSynchronizer.getPendingRequest(message.requestId);
            if (request) {
              fvdSynchronizer.removePendingRequestById(request.id);
    ...
    this.Backup = new (function () {
      ...
      setInterval(function () {
        ...
        if (isNaN(lastCheckTime) || now - lastCheckTime >= AUTOBACKUP_CHECK_INTERVAL) {
          ...
          if (hasChanges) {
            if (fvdSynchronizer.hasOwnProperty('FileSystem')) {
              selfBackup.make(function (doesBackupMade) {
                fvdSynchronizer.Prefs.set('sd.backup.has_changes', !doesBackupMade);
              });
            } else {
              chrome.runtime.sendMessage({
                action: 'makeBackup'
      ...
      this.listBackups = function (callback) {
        ...
        fvdSynchronizer.FileSystem.dirContents('backups/sd', function (dirs) {
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            ...
            function (dir, apc) {
              // read info file
              fvdSynchronizer.FileSystem.read(
                'backups/sd/' + dir + '/info.json',
                function (r, contents) {
                  if (!r) {
                    return apc();
                  }
        ...
        fvdSynchronizer.Utils.Async.chain([
          ...
          function (chainCallback) {
            if (fvdSynchronizer.hasOwnProperty('FileSystem')) {
              ...
              console.info('FileSystem is', typeof fvdSynchronizer.FileSystem);
          ...
          function (chainCallback) {
            ...
            fvdSynchronizer.FileSystem.write(
              dir + '/data.json',
              JSON.stringify(data),
              function (result) {
                if (!result) {
      ...
      this.connectToSpeeddial = function (params) {
        connectToSpeeddial(params);
      }
          ...
          function (result, data) {
            ...
            fvdSynchronizer.Utils.Async.chain([
              function (chainCallback) {
                if (apiVersion < 2) {
                  console.info(
                    "speed dial doesn't support the chunked requests, crashes can happen on a large dataset"
                  );
                  return chainCallback();
                }
    ...
    chrome.runtime.onMessage.addListener(function (message) {
      if (fvdSynchronizer.hasOwnProperty('FileSystem') && message.action === 'makeBackup') {
        fvdSynchronizer.Driver.Speeddial.Backup.make(function (doesBackupMade) {
          fvdSynchronizer.Prefs.set('sd.backup.has_changes', !doesBackupMade);
        });
      }
    });
  ...
  fvdSynchronizer.Driver.Speeddial = new Speeddial(fvdSynchronizer);
...
export default DriverSpeeddialModule;
